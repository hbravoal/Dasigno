...
using Dasigno.Infrastructure.Persistence.Utils;
using Microsoft.EntityFrameworkCore;
using System.Diagnostics.CodeAnalysis;
using System.Linq.Expressions;

namespace Dasigno.Infrastructure.Persistence.Repository.Common;
    ...
    where TId : struct
{
    ...
    private void Update(TEntity? entity, params EntityProperty[] props)
    {
        if (entity is not null)
        {
            ...
            foreach (var prop in properties)
            {
                if (!prop.PropertyType.IsImplementationOfGenericType(typeof(Domain.Interfaces.Entities.IEntity)) && (prop.PropertyType == typeof(string) || !prop.PropertyType.IsImplementationOfGenericType(typeof(IEnumerable<>))))
